...
from llm.prompts import Normal_Prompts, Improve_Prompts
...
class Gemini_Inference(Normal_Prompts, Improve_Prompts):
    ...
    def __init__(self, model_url:str = 'gemini-1.5-flash'):
        super().__init__()
        load_dotenv()
    ...
    def mode(func):
        def wrapper(self,**kwargs):
            if kwargs.get('input_data') is None:
                raise Exception('Require `input_data` to generate data')
            
            if kwargs.get('mode') is None:
                raise Exception('Require `mode` is `normal` or `improve`')
            
            # print('MRO in Inferce: ', Gemini_Inference.mro())
            ...
            if kwargs['mode'] == 'normal':
                ...
                raise NotImplementedError('input `mode`: ',{kwargs.get('mode')},' is not implemented')
...
# - Now, understand the code and re-write new testcases with suitable input arguemnts for below codes to increase the covarage value of each case and reduce the number of missing lines
